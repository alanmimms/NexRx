...
IQFrame generateTestFrame(const ToneGenerator& rf, const ToneGenerator& lo,
                          double timeS, uint32_t sequence) {
    IQFrame frame;
    frame.timestampNS = static_cast<uint64_t>(timeS * 1e9);
    frame.sequence = sequence;
    frame.flags = 0;

    // Sample RF signal
    double rf_v = rf.getSample(timeS);

    // Mix with LO (simplified QSD model)
    // Real QSD samples at 4 phases: 0°, 90°, 180°, 270°
    double lo_0 = lo.getSample(timeS);
    double lo_90 = lo.getSample(timeS + 0.25 / 14.0e6);  // 90° delay
