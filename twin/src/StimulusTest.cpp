...
double estimateFrequency(const std::vector<double>& samples, double sampleRate) {
    int crossings = 0;
...
void testToneGenerator() {
    ...
    // Test 1: Single 14.01 MHz tone, 100mV peak
    {
        ...
        constexpr double sampleRate = 100e6;
        ...
        double freq = estimateFrequency(samples, sampleRate);
...
void testRfCapturePlayer() {
    ...
    player.loadData(i_samples, q_samples, sampleRate);
