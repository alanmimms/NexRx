--[[
   NexRx Application - New UI Entry Point (Ported Original UI)
]]
...
local SliderNew = require("Slider") -- Renamed to avoid collision
local SpectrumNew = require("Spectrum")
local WaterfallNew = require("Waterfall")
...
function init()
   ...
   -- Custom Widget for the Frequency Display
   local FrequencyDisplayPorted = Widget.mkType("FrequencyDisplayPorted", Widget.Label)
   function FrequencyDisplayPorted:init(def)
      def.id = def.id or "id-rx-freq"
      Widget.Label.init(self, def)
      self.obs = def.valueObs
   end
   function FrequencyDisplayPorted:draw()
      if self.obs then
         local val = self.obs:get()
         self.text = string.format("%.3f MHz", val / 1e6)
      end
      Widget.Label.draw(self)
   end
   ...
   local wSlider = Slider.new({ valueObs = Model.rx.VFO.activeValue })
   local wVol = Slider.new({ valueObs = Model.rx.volume.DB })
   local wGain = Slider.new({ valueObs = Model.rx.RF.gainDB })
   ...
   local leftSidebar = Widget.Column{
      ...
      kids = {
         FrequencyDisplayPorted{ name = "id-rx-freq", tags = {"vfo"}, valueObs = Model.rx.VFO.activeValue, props = {fontSize = 24} },
         Widget.BridgeWidget{ name = "id-rx-slider", orig = wSlider, metrics = { stick = Stick.TLR, prefH = 20 },
         ...
         Widget.BridgeWidget{ name = "id-rx-vol", orig = wVol, metrics = { stick = Stick.TLR, prefH = 20 },
            drawArgs = { -60, 0, 0 }
         },
         Widget.Label{text = "RF Gain", metrics = {stick = Stick.TLR, prefH = 20, margin={top=10}}},
         Widget.BridgeWidget{ name = "id-rx-gain", orig = wGain, metrics = { stick = Stick.TLR, prefH = 20 },
            drawArgs = { -20, 60, 0 }
         }
      }
   }
   ...
   local centerArea = Widget.Column{
      ...
      kids = {
         Widget.BridgeWidget{ 
            name = "id-spec", tags = {"widget.Spectrum", "widget.VFOControl"},
         ...
         Widget.BridgeWidget{
            name = "id-wf", tags = {"widget.Waterfall", "widget.VFOControl"},
   ...
   local activeTagsSidebar = Widget.Column{
      name = "active-tags", tags = {"widget.Sidebar", "widget.ActiveTagsSidebar"},
      metrics = { stick = Stick.TRB, prefW = 180, margin = {left=8, right=8, top=8, bottom=8} },
      kids = {
         Widget.BridgeWidget{ name = "id-smeter", orig = wSMeter, metrics = { stick = Stick.TLR, prefH = 66 }, 
            onEvent = function(self, ev) return false end,
            drawArgs = {} 
         },
         Widget.BridgeWidget{ name = "id-active-tags", orig = wTags, metrics = { flexH = 1 } }
      }
   }
