...
bool TwinConn::initialize(const TwinConfig& cfg) {
    ...
    TCPControlClientConfig ctlCfg;
    ctlCfg.host = cfg.host;
    ctlCfg.port = cfg.controlPort;
    control = std::make_unique<TCPControlClient>(ctlCfg);
    if (!control->connect()) return false;

    UDPStreamClientConfig streamCfg;
    streamCfg.port = cfg.streamPort;
    streamCfg.receiveBufferSize = cfg.receiveBufferSize;
    stream = std::make_unique<UDPStreamClient>(streamCfg);
    if (!stream->connect()) return false;
...
bool TwinConn::startReceiving() {
    if (!connected || receiving) return false;
    stopRequested = false;
    receiving = true;
...
size_t TwinConn::pollFrames(size_t maxFrames) {
    ...
    std::lock_guard<std::mutex> lock(callbackMutex);
    if (!frameBuffer.empty() && batchCallback) batchCallback(frameBuffer);
    return count;
}

void TwinConn::receiveLoop() {
    while (!stopRequested) {
        if (pollFrames(100) == 0) std::this_thread::sleep_for(std::chrono::microseconds(100));
    }
}

std::vector<uint8_t> TwinConn::processResponse(const std::vector<uint8_t>& data, const std::string& cmdId) {
    if (data.empty()) return {};
    CborParser parser; CborValue it, array;
    if (cbor_parser_init(data.data(), data.size(), 0, &parser, &it) != CborNoError) return {};
    cbor_value_enter_container(&it, &array);
    int64_t status; cbor_value_get_int64(&array, &status);
    if (status != 0) return {};
    return data;
}
...
bool TwinConn::setVFO(double freqHz, double offsetHz) {
    uint8_t buf[128]; CborEncoder enc, arr;
...
bool TwinConn::startStream() {
    uint8_t buf[64]; CborEncoder enc, arr;
    cbor_encoder_init(&enc, buf, sizeof(buf), 0);
